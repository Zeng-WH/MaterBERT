...
## 在estimator中实现tensorboard可视化

EStimator API会根据model_fn中定义的summary operation自动序列化，无需手动hook, 而使用了hook反而会报错，应该是tensorflow内部实现中的一个兼容性问题。

在model_fn中设置相应的需要summary的值：

tf.summary.scale("name", loss)
...
同时，在tf.estimator.RunConfig中设置save_summary_steps(每隔多少部保存一次summary)
