...
class SentimentProcessor(DataProcessor):
    ...
    def get_labels(self):
        ...
        """
        return ['happy', 'anger', 'lost', 'fear', 'sad', 'other', 'anxiety']
...
class CommandProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, FLAGS.train_file)), "train")
    ...
    def get_labels(self):
        """See base class."""
        return ['ask', 'busy', 'confirm', 'deny', 'linebusy', 'myself', 'unknown']
...
def get_frozen_model(bert_config, num_labels, use_one_hot_embeddings=False):
    tf_config = tf.compat.v1.ConfigProto()
    tf_config.gpu_options.allow_growth = True
    output_node_names = ['loss/pred_prob']
    model_file = tf.train.latest_checkpoint(FLAGS.output_dir)
    with tf.Graph().as_default(), tf.Session(config=tf_config) as tf_sess:  # which solves the problems above
        label_ids = tf.placeholder(tf.int32, [None], name='label_ids')
        input_ids = tf.placeholder(tf.int32, [None, FLAGS.max_seq_length], name='input_ids')
        input_mask = tf.placeholder(tf.int32, [None, FLAGS.max_seq_length], name='input_mask')
        segment_ids = tf.placeholder(tf.int32, [None, FLAGS.max_seq_length], name='segment_ids')
...
def main(_):
    ...
    early_stopping_hook = tf.estimator.experimental.stop_if_no_decrease_hook(
        estimator=estimator,
        metric_name='eval_f1',
        max_steps_without_decrease=6 * FLAGS.save_checkpoints_steps,
        eval_dir=None,
        min_steps=0,
        run_every_secs=None,
    ...
    if FLAGS.do_train:
        train_file = os.path.join(FLAGS.output_dir, FLAGS.train_file + "_train.tf_record")
    ...
    if FLAGS.do_eval:
        ...
        eval_file = os.path.join(FLAGS.output_dir, FLAGS.dev_file + "_eval.tf_record")
    ...
    if FLAGS.do_predict:
        ...
        prefix = FLAGS.test_file.split('.')[-2] + '_' if '.' in FLAGS.test_file else ''
        predict_file = os.path.join(FLAGS.output_dir, FLAGS.test_file + "_predict.tf_record")
        predict_file_exists = os.path.exists(predict_file)
        print("###predict_file_exists:", predict_file_exists, " ;predict_file:", predict_file)
        if not predict_file_exists:  # if tf_record file not exist, convert from raw text file.
        ...
        output_predict_file = os.path.join(FLAGS.output_dir, prefix + "test_results.tsv")
        output_report_file = os.path.join(FLAGS.output_dir, prefix + "report_results.txt")
        with tf.gfile.GFile(output_predict_file, "w") as pred_writer, \
                tf.gfile.GFile(output_report_file, "w") as rep_writer:
            num_written_lines = 0
            tf.logging.info("***** Predict results *****")
            y_true = [example.label for example in predict_examples]
            y_pred = []
            for (i, (example, prediction)) in enumerate(zip(predict_examples, result)):
                probabilities = prediction["probabilities"]
                _y_pred = prediction["predictions"]
                if i >= num_actual_predict_examples:
                    break

                # _y_pred = np.argmax(probabilities)
                y_pred.append(id_to_label[_y_pred])
                information = [example.text_a, example.text_b, example.label, id_to_label[_y_pred]] \
                    if example.text_b \
                    else [example.text_a, example.label, id_to_label[_y_pred]]
                output_line = "\t".join(information + [str(class_probability) \
                                                       for class_probability in probabilities]) + "\n"
                pred_writer.write(output_line)
                num_written_lines += 1
            assert num_written_lines == num_actual_predict_examples
            rep_writer.write(classification_report(y_true, y_pred, digits=4))

    if FLAGS.do_export:
        estimator._export_to_tpu = False
        export_dir = FLAGS.export_dir if FLAGS.export_dir else \
            FLAGS.output_dir
        estimator.export_savedmodel(export_dir, serving_input_fn)

    if FLAGS.do_frozen:
        get_frozen_model(bert_config, len(label_list))
